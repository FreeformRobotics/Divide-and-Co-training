...
class Encoder(object):
	...
	def encode(self, bboxes_in, labels_in, criteria=0.5):
		...
		# boxes1 (Tensor[N, 4]), boxes2 (Tensor[M, 4]), iou (Tensor[N, M])
	...
	def decode_batch(self, bboxes_in, scores_in, nms_threshold=0.45, max_output=200):
		...
		if isinstance(bboxes_in, list):
			num_ins = len(bboxes_in)
			bboxes_l, probs_l = [], []
			for bboxes_in_, scores_in_ in zip(bboxes_in, scores_in):
				bboxes_tmp, probs_tmp = self.scale_back_batch(bboxes_in_, scores_in_)
				bboxes_l.append(bboxes_tmp)
				probs_l.append(probs_tmp)
			# [N, 8732 X S, 4/n_classes]
			bboxes, probs = torch.cat(bboxes_l, dim=1), torch.cat(probs_l, dim=1)

		elif isinstance(bboxes_in, torch.Tensor):
			num_ins = 1
		...
		for bbox, prob in zip(bboxes.split(1, 0), probs.split(1, 0)):
			...
			if num_ins == 1:
				output.append(self.decode_single(bbox, prob, nms_threshold, max_output, max_num=200 * num_ins))
			else:
				bboxes_out_l, labels_l, scores_l = [], [], []
				for bb, pr in zip(torch.chunk(bbox, num_ins, dim=0), torch.chunk(prob, num_ins, dim=0)):
					bboxes_out, labels_out, scores_out =  self.decode_single(bb, pr, nms_threshold, max_output, max_num=200)
					bboxes_out_l.append(bboxes_out.tolist())
					labels_l.append(labels_out.tolist())
					scores_l.append(scores_out.tolist())
					# import pdb; pdb.set_trace();
				# boxes, scores, labels = nms(bboxes_out_l, scores_l, labels_l, weights=None, iou_thr=0.5)
				boxes, scores, labels = weighted_boxes_fusion(bboxes_out_l, scores_l, labels_l, weights=None,
																iou_thr=0.5, skip_box_thr=0.0001)

				output.append((torch.from_numpy(boxes).float(),
...
def get_the_number_of_params(model, is_trainable=True):
